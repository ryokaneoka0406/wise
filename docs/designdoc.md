...
4. BigQuery のプロジェクトやデータセットの選択は `/init` 実行時に CLI 上で行い、必要な読み込み権限を確認する。

## メタデータ生成

- `/init` または `wise init` を実行すると、CLI 上でアクセス可能な BigQuery プロジェクトとデータセットを一覧し、ユーザーが選択したリソースのテーブル構造を取得してカラム情報やリレーションを含むメタデータを `metadata.md` として生成する。
- 選択したプロジェクトやデータセットの情報はメタデータに保存され、後続のクエリ生成・分析フローで参照される。
